# Guidelines - Projekt-Richtlinien

Willkommen im Guidelines-Repository! Dieses Projekt enthält umfassende Richtlinien und Best Practices für die Softwareentwicklung.

## 📋 Inhaltsverzeichnis

### Projektinformationen
- [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) - Verhaltensregeln für unsere Community
- [CONTRIBUTING.md](CONTRIBUTING.md) - Wie Sie zum Projekt beitragen können
- [LICENSE](LICENSE) - MIT-Lizenz
- [SECURITY.md](SECURITY.md) - Sicherheitsrichtlinien und Meldung von Schwachstellen

### Technische Dokumentation
- [ARCHITECTURE.md](docs/ARCHITECTURE.md) - Technische Architektur und Design-Entscheidungen
- [API_GUIDELINES.md](docs/API_GUIDELINES.md) - **API Design Guidelines v2.0** - RESTful API Standards
- [CODING_STANDARDS.md](docs/CODING_STANDARDS.md) - Programmierstandards und Best Practices
- [STYLE_GUIDE.md](docs/STYLE_GUIDE.md) - Code-Stil und Formatierungsrichtlinien
- [TESTING.md](docs/TESTING.md) - Test-Strategien und Qualitätssicherung
- [GIT_WORKFLOW.md](docs/GIT_WORKFLOW.md) - Git-Workflow und Branch-Strategie
- [DEPLOYMENT.md](docs/DEPLOYMENT.md) - Deployment-Prozesse und CI/CD

### GitHub-Prozesse
- [Issue-Label-Guide](.github/ISSUE_LABEL_GUIDE.md) - Erklärung der Issue-Labels
- [Pull Request Template](.github/pull_request_template/pull_request_template.md)
- [Bug Report Template](.github/issue_template/bug_report.md)
- [Feature Request Template](.github/issue_template/feature_request.md)

## 🚀 Schnellstart

1. Lesen Sie zunächst unseren [Code of Conduct](CODE_OF_CONDUCT.md)
2. Machen Sie sich mit dem [Contributing Guide](CONTRIBUTING.md) vertraut
3. Studieren Sie die [Coding Standards](docs/CODING_STANDARDS.md)
4. Verstehen Sie unseren [Git-Workflow](docs/GIT_WORKFLOW.md)

## 💡 Für neue Mitwirkende

Wenn Sie neu im Projekt sind, empfehlen wir folgende Reihenfolge:
1. [CONTRIBUTING.md](CONTRIBUTING.md) - Grundlegende Beitragsregeln
2. [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) - Community-Richtlinien
3. [docs/GIT_WORKFLOW.md](docs/GIT_WORKFLOW.md) - Wie wir mit Git arbeiten
4. [docs/CODING_STANDARDS.md](docs/CODING_STANDARDS.md) - Wie wir Code schreiben

## 🔌 Für API-Entwickler

Wenn Sie APIs entwickeln, beachten Sie:
1. [docs/API_GUIDELINES.md](docs/API_GUIDELINES.md) - **Vollständige API Design Guidelines v2.0**
2. [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) - Architektur und Integration
3. [SECURITY.md](SECURITY.md) - API-spezifische Sicherheitsaspekte

## 📞 Kontakt und Support

Bei Fragen oder Problemen:
- Erstellen Sie ein [Issue](https://github.com/your-username/Guidelines/issues)
- Lesen Sie unsere [Security Policy](SECURITY.md) für Sicherheitsfragen

## 📄 Lizenz

Dieses Projekt ist unter der MIT-Lizenz lizenziert - siehe [LICENSE](LICENSE) für Details.

---

Zuletzt aktualisiert: 2025-11-13
